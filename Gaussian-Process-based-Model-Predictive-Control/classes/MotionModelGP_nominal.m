...
classdef MotionModelGP_nominal < MotionModelGP
    %--------------------------------------------------------------------------
    ...
    methods
        function obj = MotionModelGP_nominal(d,sigmaw)
            %------------------------------------------------------------------
        ...
        function xdot = f (obj, x, u)
            ...
            % write outputs
            %--------------------------------------------------------------
            xdot  = [I_x_dot; I_y_dot; psi_dot; V_vx_dot; V_vy_dot; psi_dot_dot];
        ...
        function gradu = gradu_f(obj, x, u)
            ...
            %   Continuous time dynamics.
            %   out:
            %       gradu: <n,m> gradient of xdot w.r.t. u
            %------------------------------------------------------------------
            params = [obj.M obj.I_z obj.l_f obj.l_r obj.deltamax ...
                obj.maxbrakeWForce obj.maxmotorWForce obj.c_f obj.c_r]';
        ...
        function generate_grad_functions(obj)
            ...
            %       nomModel = MotionModelGP_nominal(@(z)deal(zeros(3,1),zeros(3)), zeros(3));
            %       nomModel.generate_grad_functions()
            %------------------------------------------------------------------
            syms I_x I_y vpsi V_vx V_vy psi_dot real
            x = [I_x I_y vpsi V_vx V_vy psi_dot]';
            syms delta T real
            u = [delta T]';
            syms M I_z l_f l_r deltamax maxbrakeWForce maxmotorWForce c_f c_r real
            ...
            xdot = obj.f(x,u);
            gradx = jacobian(xdot,x);  % Remove transpose - jacobian gives (n,n)
            gradu = jacobian(xdot,u);  % Remove transpose - jacobian gives (n,m)
            ...
            disp('FINISHED! functions singletrack_gradx_f and singletrack_gradu_f generated!!')
        end
        ...
        function [F_y_f, F_y_r] = getTireForces(obj, x, u)
            %------------------------------------------------------------------
            %   Extract lateral tire forces from the linear tire model
            %   out:
            %       F_y_f: front lateral tire force [N]
            %       F_y_r: rear lateral tire force [N]
            %------------------------------------------------------------------
            ...
            % Linear tire model forces (in wheel coordinates)
            W_Fy_r = c_r * a_r;
            W_Fy_f = c_f * a_f;
            
            % Convert to vehicle coordinates (z-axis points up)
            F_y_r = -W_Fy_r;
            F_y_f = -W_Fy_f;
        end
    end
