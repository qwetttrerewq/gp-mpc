...
fprintf('\n========================================\n');
fprintf('6-State Single Track Vehicle NMPC Test (TRUE Model)\n');
fprintf('========================================\n\n');
...
radius = 20;        % [m] circle radius (SAME as nominal)
...
n = trueModel.n;  % 6 states
m = trueModel.m;  % 2 inputs
...
x0 = [0; 0; 0; v_ref; 0; 0];  % start at origin, zero heading, reference velocity, zero slip
...
% Input: Torque
subplot(4,2,4)
stairs(out.t(1:kmax), out.u(2,:), 'b-', 'LineWidth', 1.5); hold on;
yline(u_ub(2), 'r--', 'LineWidth', 1);
yline(u_lb(2), 'r--', 'LineWidth', 1);
grid on; ylabel('T [-]'); title('Torque Gain');
legend('Command', 'Bounds', 'Location', 'best');
...
sgtitle('6-State Single Track NMPC Test: Circular Path Tracking (TRUE Model - Pacejka Tires)');
